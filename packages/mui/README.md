...
- `3.0.0`부터 React 19 전용입니다(MUI 9). React 18은 지원하지 않습니다.
- `2.x`는 MUI 9 + React 18·19를 지원합니다.
- 기존 MUI 7 지원 버전은 `1.x` 버전 또는 `mui7` 브랜치에서 유지합니다.
...
- `3.0.0`부터 React 19 전용입니다. React 18은 지원하지 않습니다.
- `peerDependencies.react`는 `^19`로 제한됩니다. React 18 트리에는 설치되지 않습니다.
- `forwardRef`를 제거하고 React 19 ref-as-prop 패턴으로 전환한 데 따른 의도된 breaking change입니다.
- 공개 컴포넌트의 ref 전달·id 생성 동작은 회귀 없이 유지됩니다.
- useId 출력 포맷이 변경되었습니다(breaking): 생성 id에서 `mantine-` prefix를 제거했습니다. 생성 id 문자열에 의존하던 코드는 영향을 받습니다.

## 추가 정보

- 모든 컴포넌트는 ref를 일반 prop으로 직접 전달받습니다(React 19 ref-as-prop, `forwardRef` 불필요). `<Center ref={...} />` 형태의 기존 사용 코드는 동일하게 동작합니다.
- MUI의 `sx` prop과 완벽 호환
